...
* `XOR`: Bitwise XOR of two equal-length bit strings.
...
* **Signature**: A UOV signature `s in F_q^{n_uov}` satisfying `P(s) = t` for a
  target `t in F_q^m`. (Like standard NIST UOV, we hash-then-sign; unlike it,
  the target is `t = Tag(Com(nf, t, r), x)` rather than a hash of a message.)
...
`Tag` embeds the issuer's refund `x` into the target. The issuer signs `Tag(c,
x)` with the UOV trapdoor, so a single signature jointly binds the committed
contents `(nf, t)` through `c` and the refund `x` the issuer granted when it
produced the signature. The refund is bound in the public tag rather than
inside the commitment, which lets the issuer add credits at signing time
without the client re-committing; at initial issuance `x = 0`.
...
Here `Com` is realized by `KP800`, a TurboSHAKE-style {{?RFC9861}} sponge over
the reduced-round permutation `Keccak-p[800, 12]`, modeled as a random oracle
and domain-separated by `dst`. `Tag` is the XOR-embedding map `Tag(c, x) = c XOR
EmbedRefund(x)`, folding the public refund into the target's low bytes with no
second hash. In the Spend relation ({{prove-spend}}), V1 recomputes the old
commitment `c` in-circuit (its output stays hidden) and V2 recomputes the change
commitment `c'` (its output is the public one), so a spend evaluates
`Keccak-p[800, 12]` twice in-circuit -- the dominant proving cost, which the MQ
instantiation ({{mq-commitment}}) removes entirely. The `800`-bit permutation
(rather than TurboSHAKE128's `1600`-bit `Keccak-p[1600, 12]`) halves the
in-circuit state, at the cost of a tighter single-block budget that constrains
`dst` (see below).
...
Com(nf, t, r):
  Input:
    ...
    - r: Randomness (rho bits)
  Output:
    - c: Commitment (m*8 bits, larger than 2*lambda bits)

  Steps:
    1.  return KP800(0xAC || dst || nf || <t>_L || r, m)  // 44 bytes
~~~

**Tag.** `Tag` derives the UOV target from a commitment `c` and a public refund
`x`:
...
Tag(c, x):
  ...
  Steps:
    1.  return c XOR EmbedRefund(x)     // XOR-embed the refund
~~~

`EmbedRefund(x)` returns an `m*8`-bit string that is zero everywhere except its
low `L` bits, which hold the refund encoding `<x>_L`; XORing it into `c` folds
the public refund into the target's low `L/8` bytes. At initial issuance `x =
0`, so `EmbedRefund(0) = 0` and `Tag(c, 0) = c`.

Only `Com` is hashed; its preimage has fixed-length fields, so it parses
unambiguously. `KP800` is a TurboSHAKE-style {{?RFC9861}} sponge over the
reduced-round permutation `Keccak-p[800, 12]` (the 800-bit Keccak permutation
restricted to 12 rounds); each call absorbs or squeezes 544 bits (`68` bytes) of
the state as the rate while the remaining 256 bits as the capacity are never
exposed. The preimage (label &#124; `dst` &#124; `nf` &#124; `<t>_L` &#124; `r`)
is `1 + 10 + 32 + 8 + 16 = 67` bytes `= rate - 1` and the `m*8`-bit output also
fits, so `Com` is a single `Keccak-p[800, 12]` permutation; this tight budget is
why `dst` is fixed to `10` bytes. `Tag` does no hashing: it XORs `EmbedRefund(x)`
into `c`.

**Security.** Binding follows from the `m*8`-bit output (collision resistance
above `lambda`, as `m*8 > 2*lambda`); hiding from the `rho = 128` bits in `r`;
both in the ROM. For unforgeability, note that `x` sits in the target *outside*
the hash, so `c XOR EmbedRefund(x)` is not a fresh random-oracle image and
security does not reduce to plain UOV EUF-CMA. It rests instead on a **one-more
UOV-inversion** assumption -- no adversary can produce more valid `(commitment,
opening, signature)` triples than it obtained from the signing oracle -- the
multivariate analogue of one-more-RSA-inversion {{BNPS03}} and the flavour
underlying the MQ instantiation's quadratic-claw {{BFMRSV25a}}. A deployment
wanting the conservative UOV EUF-CMA reduction MAY fold `x` into the hash (`c =
Com(nf, t, r, x)`, `Tag = identity` on the full `m` bytes), losing sign-time
refunds.
...
|----------|-------|--------|
| `Com(nf, t, r)` | label `0xAC` = `1`, `dst: 10`, `nf: nu/8 = 32`, `<t>_L: L/8 = 8`, `r: rho/8 = 16` (preimage `67 = rate - 1`) | `c: m = 44` |
| `Tag(c, x)` | `c: 44` |
...
trusted setup. Unlike the hash commitment of {{hash-commitment}} (which uses
`Keccak-p[800, 12]` so its `Com` is cheap to prove in-circuit), this expansion
is never proven in zero-knowledge, so it uses standard SHAKE128 (FIPS 202)
{{FIPS202}} -- the more conservatively reviewed XOF:
...
The two instantiations of {{instantiations}} were measured in a reference
implementation on the VOLE-in-the-Head NIZK at `lambda = 128` (degree-16 circuit
for the hash commitment, degree-2 for the MQ commitment), single-threaded on a
laptop-class aarch64 core. The numbers are indicative and meant for relative
comparison, not as normative bounds.

**Runtime.** Prove is a client cost and verify an issuer cost.

| Operation | Hash (Keccak-p[800], hash commitment) | MQ (BFMRSV25a, MQ commitment) |
|-----------|------------------------|---------------------|
| `IssueRequest` (prove) | 92 ms | 24 ms |
| `IssueResponse` (verify) | 45 ms | 65 ms |
| `ProveSpend` | 268 ms | 85 ms |
| `VerifyAndRefund` (verify) | 162 ms | 57 ms |

**Communication.** Proof size dominates each message; the public `nf`, `d`, and
`c'` add a few dozen bytes.

| Proof | Hash | MQ |
|-------|-----------|----------|
| Issuance `pi_init` | 10.4 KiB | 4.3 KiB |
| Spend `pi` | 16.0 KiB | 9.0 KiB |

The MQ commitment removes the in-circuit Keccak, so the spend proof is both
smaller (about half the size) and faster than the hash commitment -- about 3x
faster to prove and to verify -- despite its widened UOV map (`n_uov = 333`, `m
= 131`). The one cost the MQ instantiation pays is `KeyGen`, which builds the
widened-UOV trapdoor and runs to several seconds (a one-time issuer cost).
Reaching this spend speed with such a large map requires care: the `M` public
quadratic forms are folded into one via a random linear combination using
per-form 256-entry lift tables, so the fold is table lookups plus additions
rather than roughly `m * n_uov^2 / 2 ~ 7M` field multiplications; without that,
the fold alone dominates and the spend proof is ~10x slower. The hash commitment
needs no such fold but pays for a degree-16 Keccak circuit, making its proofs
larger and slower.
