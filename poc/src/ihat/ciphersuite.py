...
class P256Group(PrimeOrderGroup[P256SHA256]):
    ...
    def SerializeElement(self, element: Element[P256SHA256]) -> bytes:
        ...
        x = int(element._point.x)
        y = int(element._point.y)
    ...
    def DeserializeElement(self, buf: bytes) -> Element[P256SHA256]:
        ...
        y_squared = (pow(x, 3, FIELD_MODULUS) + CURVE_A * x + CURVE_B) % FIELD_MODULUS
        y = pow(y_squared, (FIELD_MODULUS + 1) // 4, FIELD_MODULUS)
        if y * y % FIELD_MODULUS != y_squared:
            raise DeserializeError("P-256 encoding is not on the curve")
        if y & 1 != buf[0] & 1:
            y = FIELD_MODULUS - y
        try:
            element: Element[P256SHA256] = Element(EccPoint(x, y, curve="p256"))
        except ValueError as error:
            raise DeserializeError("invalid P-256 element") from error
        if element.is_identity():
