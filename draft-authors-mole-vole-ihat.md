...
Let `P[0], ..., P[N-1]` denote the Anchor Set. To redeem a token, the Client
...
The idea is to extend the witness with a "selector" vector `b[0], ..., b[N-1]`
...
- `nu`: the nullifier length in bytes. Its value SHALL be `24`.

- `rho`: the commitment opening length in bytes. Its value SHALL be `16`.
...
F_q^rho` and running the following procedure:

~~~~pseudocode
Com(nf, ctx_iss, ctx_red, r):
  Input:
    - nf: the nullifier, in F_q^nu
    - ctx_iss: issuance context, in F_q^16
    - ctx_red: redemption context, in F_q^16
    - r: the commitment opening, in F_q^rho

  Output:
    - t: the commitment, in F_q^m

  Steps:
    1. state := "mouss" || VERSION || 0^(26 - rho) || r ||
                ctx_iss || ctx_red || 0^(36 - nu)  || nf
    2. t := KP800(state)[0..m]
    3. return t
~~~~
...
inputs relative to {{IHAT}}: `nu` is 24 bytes, whereas {{IHAT}} uses a 32-byte
...
Relation IssueRelation(t, ctx_iss):
  ...
  Witness:
    - nf: nullifier, in F_q^nu
    - r: commitment opening, in F_q^rho
    - ctx_red: redemption context, in F_q^16
...
Select(b, P):
  Input:
    - b: selector vector, in F_q^N
    - P: Anchor Set, a sequence of N maps from F_q^n to F_q^m
...
for all `u in [N-1]`. The redemption relation is defined as follows:
...
Relation RedeemRelation(P, nf, ctx_iss, ctx_red):
  ...
  Constraints:
    ...
    - sum(b[0..u+1]) * b[u+1] = 0            for u in [N-1]
    - sum(b) = 1
~~~~
...
FinalizeRedeem(P, request, ctx_iss, ctx_red):
  ...
  Output:
    - nf: the nullifier of the redeemed token, in F_q^nu, or INVALID
